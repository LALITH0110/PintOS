...
// Function to check and unblock threads if their sleep duration has ended
 void
 checkInvoke(struct thread *t, void *aux UNUSED)
   {
    // Check if the thread is blocked and has remaining blocked ticks.
 ...
 * Get thread structures from list elements (list_entry(a, struct thread, elem)->priority)
 */

 // Functions I have created all at one place-
...
thread_hold_lock(struct lock *lock)
{
  ...
  // Add the lock to the list of locks held by the current thread.
  list_insert_ordered(&cur->locks_holding, &lock->elem, lock_compare, NULL);

  // Update the thread's priority if necessary.
...
thread_remove_lock(struct lock *lock)
{
  ...
  // Iterate through the list of locks held by the current thread.
...
{
  // Ensure that both elements are not null.
  if (a == NULL || b == NULL)
    return false;
...
thread_update_priority(struct thread *t)
{
  ...
  // Check if the thread is holding any locks.
  if (!list_empty(&t->locks_holding))
  {
    // Sort the locks by max priority.
    list_sort(&t->locks_holding, lock_compare, NULL);
    
    // Get the max priority of the locks.
    struct list_elem *e = list_front(&t->locks_holding);
    if (e != NULL)
    {
      lock_pri = list_entry(e, struct lock, elem)->max_priority;
      
      // Update the thread's priority if the lock's max priority is higher.
      if (max_pri < lock_pri)
        max_pri = lock_pri;
    }
  }
...
thread_unblock (struct thread *t) 
{
  ...
  //Since this function Unblocks a thread by adding it to the ready list in order of priority.
  //This line which I added inserts the thread into the ready list based on priority
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  // Add the current thread to the ready list if it's not the idle thread
  if (cur != idle_thread) 
...
{
  if (thread_mlfqs)
    return; // MLFQS mode does not support manual priority setting.
  ...
  {
    intr_set_level(old_level);
    return; // Invalid priority, do nothing.
  }
  ...
  if (list_empty(&cur->locks_holding) || new_priority > old_priority)
  {
    ...
    thread_yield(); // Yield if the new priority is lower than the highest ready thread.
  }
  ...
  // // Yield the CPU if the new priority is lower than the highest ready thread
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  old_level = intr_disable ();
  list_insert_ordered(&all_list, &t->allelem, (list_less_func *)&thread_compare, NULL); //previous assignment
  intr_set_level (old_level);
}
               ...
               thread_func *function, void *aux) 
{
  ...
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
      ...
      if (strcmp(curr->name, "idle") != 0) {
          thread_count++;
          if (curr->priority < min_priority) {
              min_priority = curr->priority;
              lowest_name = curr->name;
          }
      }
  }
